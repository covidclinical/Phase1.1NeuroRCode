...
neuro_icds_9 <- neuro_icds_9 %>% 
  within(., ICD10_DESCRIPTION[icd == 'V41'] <- 'Problems with special senses and other special functions') %>% 
  within(., `Neurological Disease Category`[icd == 'V41'] <- 'Vision/smell/taste') %>%
  within(., ICD10_DESCRIPTION[icd == '781'] <- 'Symptoms involving nervous and musculoskeletal systems') %>% 
  within(., `Neurological Disease Category`[icd == '781'] <- 'Neuropathy') %>% # and Consciousness
  within(., ICD10_DESCRIPTION[icd == '437'] <- 'Other and ill-defined cerebrovascular disease') %>%
  within(., ICD10_DESCRIPTION[icd == '780'] <- 'General symptoms') %>%
...
```{r}
colnames(neuro_icds_9)
neuro_icds_9 %>% 
  select(`Neurological Disease Category`, icd, ICD10_DESCRIPTION) %>% 
  distinct() %>% 
...
diag_icd_9 <- diag_ana %>% 
  ...
  select(- icd_version) %>% 
  pivot_wider(names_from = severe, values_from = count) %>%
  right_join(neuro_icds_9 %>% select(- c(ICD10_CODE, ICD9_CODE, ICD9_DESCRIPTION)), by = 'icd') %>% 
  distinct() %>% 
  left_join(demo_ana, by = 'siteid') %>% 
...
age_group_desc <- demo_all %>% 
  ...
  rename('Characteristics' = age_group)
