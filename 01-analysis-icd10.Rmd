...
code_prevalence <- diag_icd_10 %>% 
  ...
  mutate(percent_pats_country = sum(num_patients_icd)/sum(num_patients_all)) %>% 
  ungroup()
...
diff_code <- code_prevalence %>% 
  ...
  pivot_wider(names_from = time, values_from = percent_pats_site, 
              ...
              values_fill = 0) %>% 
  mutate(percent_diff = `After admission` - `Before admission`,
         loc_type = 'Site', location = tolower(location), .keep = 'unused') %>% 
...
diff_heat <- diff_code %>% 
  ...
  theme(panel.spacing.x = unit(10, "points"),
        plot.margin = unit(c(1,0.2,0.2,3), "lines"),
...
plot_grid(diff_heat, icd_prevalence_plots, rel_heights = c(1, 0.9),
          ncol = 1, labels = 'AUTO') %>%
  ggsave('figs/icd_prevalence_AB.png', ., height = 7.5, width = 9.7)
```
