...
sum(is.na(diag_icd_10))
check_time_site <- diag_icd_10 %>% 
  distinct(siteid, time) %>% 
  add_count(siteid) %>% 
  filter(n == 2)

diag_all %>% 
  group_by(siteid) %>%
  summarise(num_site_time = sum(num_patients_all_since_admission), .groups = 'drop')

diag_all %>% 
  group_by(siteid) %>%
  summarise(num_site_time = sum(num_patients_all_before_admission), .groups = 'drop')
```
...
diff_code <- code_prevalence %>% 
  ...
  pivot_wider(names_from = time, values_from = percent_pats_site, 
              id_cols = c(siteid, icd, `Neurological Disease Category`), 
              values_fill = 0) %>% 
...
diff_heat <- diff_code %>% 
  ...
  facet_grid(rows = vars(`Neurological Disease Category`), space = 'free', scale = 'free') +
  heat_theme_bottom() +
  theme(plot.margin = unit(c(1,0.2,0.5,5), "lines")) +
  NULL
diff_heat

ggsave('figs/icd_diff_heatmap.png', diff_heat, height = 4, width = 8)
```
...
my_t <- function(icdi){
  ...
  data.frame(
    icd = icdi,
    t.test(diff_icd$`After admission`, diff_icd$`Before admission`, paired = TRUE) %>% 
    broom::tidy()
  )
}
...
prevalence_stats %>% 
  ...
  mutate(ci = paste0('(', round(conf.low, 1), ', ', 
                     round(conf.high, 1), ')')) %>% 
...
sorted_icds <- code_prevalence %>% 
  ...
  pivot_wider(names_from = time, values_from = percent_pats_site, values_fill = 0) %>% 
  group_by(full_icd) %>% 
...
total_icd <- icd_time %>% 
  ...
  theme(legend.position = c(0.3, 0.15),
        ...
        plot.margin = margin(t = 1, r = 0.5, unit = 'lines')) + 
...
```{r}
plot_grid(diff_heat, icd_prevalence_plots, rel_heights = c(1, 0.9),
          ncol = 1, labels = 'AUTO') %>%
  ggsave('figs/icd_prevalence_AB.png', ., height = 7, width = 9.4)
```
...
filtered_obs_exp <- contingency_df %>% 
  ...
  mutate(
    ...
    presentation = case_when(
      lower > 1 & `P value (FDR)` < 0.05 ~ '#ca562c', 
      upper < 1 & `P value (FDR)` < 0.05 ~ '#009392',
      TRUE ~ 'grey20'
    )) %>% 
  {.}
```
...
enrichment_plot_before <- plot_enrich(
  ...
  plot_obs_exp_right %>% filter(time == 'Before admission') %>% slice(match(sorted_icds, full_icd)))
