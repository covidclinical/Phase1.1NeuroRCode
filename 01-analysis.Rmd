...
fish_obs_exp %>% 
  ...
  left_join(contingency_df, by = 'full_icd') %>% 
...
enrichment_plot_left <- plot_obs_exp_left  %>% 
  ...
  geom_pointrange(
    aes(y = lestimate, ymin = llower, ymax = lupper, group = full_icd),
...
enrichment_plot_right <- plot_obs_exp_right %>%
  ggplot(aes(x = fct_relevel(full_icd, sorted_icds), y = Observed)) +
  geom_segment(aes(xend =  fct_relevel(full_icd, sorted_icds), yend = Expected), 
                color = filtered_obs_exp$presentation) +
  geom_point(aes(y = Observed), shape = 16, color = 'grey20') +
  geom_point(aes(y = Expected), shape = 17, color = 'grey20') +
  ...
  geom_text(
    data = . %>% 
      filter(!(full_icd %in% over_severe_icds)),
    nudge_y = -7, aes(label = round(over_sev, 0)), size = 2.5) +
  geom_text(
    data = . %>% 
      filter(full_icd %in% over_severe_icds), 
    nudge_y = 7, aes(label = round(over_sev, 0)), size = 2.5)

enrichment_plot_before <- cowplot::plot_grid(enrichment_plot_left, enrichment_plot_right,
                                rel_widths = c(1, 2.8))
# enrichment_plot_before
# ggsave('figs/icd_severe_bef.png', enrichment_plot_before, width = 9.5, height = 5)
...
```{r warning=FALSE, fig.width = 12, fig.height = 8, echo=FALSE}
plot_obs_exp_right <- plot_obs_exp %>% filter(subtype == 'Sqrt(number of honorees)') %>% 
  pivot_wider(names_from = type)
plot_obs_exp_left <- plot_obs_exp %>% filter(subtype != 'Sqrt(number of honorees)') %>%
  pivot_wider(names_from = type)
...
enrichment_plot_right <- plot_obs_exp_right %>%
  ...
  theme(
    ...
    legend.key.height = unit(3, 'mm'),
    legend.key.width = unit(3, 'mm'),
    plot.margin = margin(20.5, 5.5, 8.5, 2, unit = 'pt')
  ) +
  scale_y_sqrt(
    breaks = c(100, 1000, 3000),
    expand = expansion(add = c(0, 10))) +
...
enrichment_plot_after <- cowplot::plot_grid(enrichment_plot_left, enrichment_plot_right,
                                rel_widths = c(1, 2.8))
