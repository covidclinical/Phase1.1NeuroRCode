...
diff_code <- code_prevalence %>% 
  ungroup() %>% 
  select(time, siteid, icd, percent_pats_site, `Neurological Disease Category`) %>%
...
diff_heat <- diff_code %>% 
  ...
  facet_grid(rows = vars(`Neurological Disease Category`), space = 'free', scale = 'free') +
  heat_theme_bottom() +
  theme(plot.margin = unit(c(1,10,0.5,14), "lines")) +
  NULL
diff_heat
...
prevalence_stats %>% 
  write_csv('results/icd9_prevalence_stats')
```
...
```{r}
plot_grid(diff_heat, icd9_prevalence_plots, rel_heights = c(1, 0.9),
          ncol = 1, labels = 'AUTO') %>%
  ggsave('figs/icd9_prevalence_AB.png', ., height = 7, width = 9.4)
```
...
enrichment_plot
ggsave('figs/icd9_severe_before_after.png', enrichment_plot, width = 9.5, height = 7)
```
