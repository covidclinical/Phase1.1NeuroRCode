...
age_radar$name <- factor(age_radar$name, levels = unique(age_radar$name))
  
Races <- rev(c('White', 'Black', 'Hispanic/Latino', 'Asian', 'American Indian', 'Hawaiian/Pacific Islander', 'Other/Unknown'))
race_perc <- demo_perc %>% 
  filter(abs(unknown_other_race) < 1) %>% 
  select(Country, num_patients_all, num_patients_ever_severe, all_of(races), unknown_other_race)
...
age_plot <- age_radar  %>% 
  ...
  coord_flip() +
  facet_grid(rows = vars(fct_relevel(Country, countries))) +
  ...
  theme(legend.position = 'None',
        ...
        plot.margin = margin(10.5, 5.5, 5.5, 5.5, 'pt')) +
  NULL
```
...
gender_race <- plot_grid(gender_plot, race_plot, ncol = 1,
          rel_heights = c(0.897, 1), labels = 'AUTO')
comp_plots <- plot_grid(gender_race, age_plot, ncol = 2, rel_widths = c(1.2, 1),
          labels = c('', 'C'))
comp_plots
ggsave('figs/composite_plot.png', comp_plots, height = 6.7, width = 8)
```
